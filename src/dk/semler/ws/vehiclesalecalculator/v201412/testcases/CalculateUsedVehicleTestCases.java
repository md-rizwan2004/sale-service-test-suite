...
public class CalculateUsedVehicleTestCases {
	...
	public CalculateUsedVehicleTestCases(String inputvehicleid, String expectedtotalpriceinclvat){
	...
	public static void setUpBeforeClass() throws Exception {
		...
		if (ServiceEnvironment.isProduction()) {
			//TODO add production end point here
	...
	public void setUp() throws Exception {
		...
		economyType.setVEHICLESALEPRICE(new BigDecimal(0.00));
		...
		//unmarshal.setProperty(, value);
		StringBuffer xmlStr = new StringBuffer(vehicleSaleCalculatorService.executeService(vehiclePriceRequest).getOutputMessage());
		responseMessage = (dk.semler.ws.vehiclesalecalculator.v201412.calculateusedvehicle.response.MESSAGE) 
				unmarshal.unmarshal(new StreamSource(new StringReader(xmlStr.toString())));
	}

	@Test
	public void VehicleGrandTotalPriceInclVATShouldMatch() {
		assertEquals(new BigDecimal(expectedtotalpriceinclvat).setScale(2), 
				responseMessage.getRESPONSE().getCALCULATION().getGRANDTOTALPRICE().getEXCLTAXINCLVAT().setScale(2));
	}
